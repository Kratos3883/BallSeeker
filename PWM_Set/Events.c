...
extern bool error_flag, parate_flag, retro_flag;		//Se ha recibido centroide, pasa a UBICAR_PELOTA

extern unsigned char rChar;

extern unsigned char r_n;//motor rojo-negro
extern unsigned char v_b;//motor verde-blanco
...
void  AS2_OnRxChar(void)
{
	...
	AS2_RecvChar(&rChar);
	if(rChar==0xFF)
		bypass1=1;	//Recibi trama desde el bluetooth
	else if(bypass1)
	{	
		r_n=rChar;
		bypass1=0;
